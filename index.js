...
const routes = [
  [
    "/gs/",
    "gs",
    path.join(__dirname, "pages", "gs.html"),
    (str) => str.replace("$gs", buildGames(require("./gs.json"))),
  ],
...
routes.forEach(([path, id, filePath, fn]) =>
  fastify.get(path, (request, reply) =>
    reply.type("html").send(
      render({
        main: fn
          ? fn(fs.readFileSync(filePath, "utf-8"))
          : fs.readFileSync(filePath, "utf-8"),
        theme: request.cookies["appearance"] || config.appearance || "terminal",
...
fastify.get("/gateway/", (request, reply) =>
  gateway(request, reply, base64.decode(request.query.url || ""))
);
fastify.post("/gateway/", (request, reply) =>
  gateway(
    request,
    reply,
    typeof request.body == "object" ? request.body.url : request.body
  )
);
...
fastify.post("/suggestions/", (request, reply) => {
  ...
  try {
    https
      .request(
        `https://duckduckgo.com/ac/?q=${encodeURIComponent(val)}`,
        {},
        async (resp) => {
    ...
    fastify.use((req, res, next) => {
      if (req.url.startsWith(corrosion.prefix)) {
        if (config.authorization && req.headers.cookie) {
          ...
          if (
            !cookies.has(config.authorization.name) ||
            cookies.get(config.authorization.name) != config.authorization.value
          ) {
            res.status(401);
            return res.send("");
          }
...
function gateway(request, reply, val) {
  const url = isUrl(val)
    ? new URL(/^http(s?):\/\//.test(val) ? val : "http://" + val).href
    : `${
        engines[request.cookies.engine || config.engine] || engines.google
      }${val}`;
  if (config.authorization)
    reply.header(
      "Set-Cookie",
      `${config.authorization.name}=${config.authorization.value}; Domain=${request.headers.host};`
    );
